...
from wealthboard.etf.adapters.driving.owned_router import OwnedETFRouter
...
from wealthboard.etf.adapters.decorators.cache_history_repository import CacheHistoryRepository
from wealthboard.etf.app.use_cases.multiple_metrics import MultipleMetrics
from fastapi import FastAPI
from wealthboard.etf.adapters.driving.etf_history_router import ETFHistoryRouter
from wealthboard.ui.web.web_router import router as web_router
...
history_etf_cache = CacheHistoryRepository(repo_history)

# === SERVICES === 
owned_etf_service = OwnedETFService(owned_etf_cache, price_provider)
etf_service = ETFService(repo_etf)
history_service = ETFHistoryService(history_etf_cache, price_provider)
...
controller_history = ETFHistoryRouter(history_service)
controller_owned = OwnedETFRouter(owned_etf_service)

app.include_router(controller_history._router, prefix="/etf-history")
app.include_router(controller_owned._router, prefix="/etf-owned")
